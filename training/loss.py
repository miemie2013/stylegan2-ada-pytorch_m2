...
class StyleGAN2Loss(Loss):
    ...
    def run_G(self, z, c, sync, dic, pre_name):
        with misc.ddp_sync(self.G_mapping, sync):
            z.requires_grad_(True)
            ws = self.G_mapping(z, c, dic, pre_name + '_mapping')
            dws_dz = torch.autograd.grad(outputs=[ws.sum()], inputs=[z], create_graph=True,
                                         only_inputs=True)[0]
            dic[pre_name + 'dws_dz'] = dws_dz.cpu().detach().numpy()
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain, dic):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws = self.run_G(gen_z, gen_c, sync=(sync and not do_Gpl), dic=dic, pre_name=phase) # May get synced by Gpl.
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, _gen_ws = self.run_G(gen_z, gen_c, sync=False, dic=dic, pre_name=phase)
